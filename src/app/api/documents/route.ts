...
export async function PUT(req: Request) {
  ...
  if (!id || !file || file.size === 0) return NextResponse.json({ error: "ID et fichier requis" }, { status: 400 });

  let fileUrl = null;
  try {
    const { put } = await import("@vercel/blob");
    const ext = file.name.split(".").pop() || "pdf";
    const blob = await put(`documents/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`, file, { access: "public" });
    fileUrl = blob.url;
  } catch { return NextResponse.json({ error: "Upload échoué" }, { status: 500 }); }

  await db.update(documents).set({ fileUrl }).where(eq(documents.id, id)).run();
